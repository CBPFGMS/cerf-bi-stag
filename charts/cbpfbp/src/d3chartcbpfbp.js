(function d3ChartIIFE() {
	...
	function d3Chart() {
		...
		function draw(dataDonors, dataCbpfs) {
			...
			var maxXValue = d3.max([d3.max(top10Cbpfs, function(d) {
			...
			top10Donors.forEach(function(d) {
				...
				d.donations.forEach(function(e) {
					...
					var cbpfName = otherCbpfs && otherCbpfs.cbpfList.indexOf(e.cbpf) > -1 ? "Other CERFs" : e.cbpf;
			...
			function drawBeeswarm(category) {
				...
				circles.on("mouseover", function(d) {
						...
						var flag = category === "CERFs" ? "<br>" : "<img src='" + imageSource + "' height='24' width='24' style='padding:0px;'><br style='line-height:180%;'/>";
			...
			function mouseenterBarsCbpfs(d, barNumber) {
				...
				var cbpfText = d.cbpf === "Other CERFs" ? "CERFs" : "CBPF";
			...
			function mouseenterLinks(d) {
				...
				var thisCbpfName = d.target.cbpf === "Other CERFs" ? "Other CBPFs (" + d.target.cbpfNameInOthers + ")" : d.target.cbpf;
			...
			function highlightBeeswarm(datum, type) {
				...
				var dataList = datum.donor === "Other Donors" ? "donorsList" : datum.cbpf === "Other CERFs" ? "cbpfList" : null;
				var thisCircle = [];
				...
				verticalLine.attr("x1", beeswarmXScale(datum.totalPaidPlusPledge))
					...
					.attr("y1", Object.keys(datum)[0] === "donor" ? beeswarmYScale("Donors") : beeswarmYScale("CERFs"))
					.attr("y2", beeswarmPanel.height - beeswarmPanel.padding[2])
					.style("opacity", 1);

				if (datum.donor === "Other Donors" || datum.cbpf === "Other CERFs") {
					verticalLine.style("opacity", 0)
				};
			...
			function highlightBeeswarmLinks(source, target) {
				...
				beeswarmPanel.main.selectAll("circle")
					.style("opacity", function(d) {
						...
						if (target.cbpf === "Other CERFs" && target.cbpfNameInOthers === d.cbpf) {
							found = true;
							theseCircles.push({
								x: d3.select(this).attr("cx"),
								y: d3.select(this).attr("cy"),
